...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const existing = await qGet('SELECT id FROM products WHERE id = ?', id);
  ...
  const current = (await qGet<any>('SELECT * FROM products WHERE id = ?', id))!;
  const merged = {
  ...
  await qRun(
    `UPDATE products SET name=?, description=?, category_id=?, price=?, promo_price=?, stock=?, unit=?, sku=?, featured=?, available=?, image_url=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
  ...
  const row = await qAll('SELECT p.*, c.name AS category_name FROM products p LEFT JOIN categories c ON c.id = p.category_id WHERE p.id = ?', id);
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  const res = await qRun('DELETE FROM products WHERE id = ?', id);
