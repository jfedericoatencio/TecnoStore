...
export async function POST(req: Request) {
  ...
  const dup = qAll('SELECT id FROM categories WHERE lower(name) = lower(?)', name);
  if (dup.length > 0) {
  ...
  const res = qRun('INSERT INTO categories (name, sort_order) VALUES (?, ?)', name, Number(max) + 1);
  const row = qAll('SELECT * FROM categories WHERE id = ?', lastId(res))[0];
