...
export async function GET() {
  ...
  const categories = Number((await qGet<any>('SELECT COUNT(*) AS c FROM categories'))!.c);

  const ordersToday = Number(
    (await qGet<any>(`SELECT COUNT(*) AS c FROM orders WHERE date(created_at) = CURRENT_DATE`))!.c
  );
  ...
  const totalOrders = Number((await qGet<any>('SELECT COUNT(*) AS c FROM orders'))!.c);

  const recentOrders = await qAll<Order>(
    'SELECT id, number, customer_name, total, status, created_at FROM orders ORDER BY created_at DESC, id DESC LIMIT 8'
  );
  const lowStockProducts = (await qAll<Product>(
  ...
  return Response.json({
    stats: {
      ...
      availableProducts: Number(totals?.available_products ?? 0),
